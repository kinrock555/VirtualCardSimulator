...
テーブルの周囲には部屋の床・壁が常に表示され、カメラをどこに動かしてもテーブルの外側が何もない空間にならないようになっています。用意されている4種類の部屋背景から選べます。

- **無地** : 明るすぎないグレーの床と壁だけのシンプルなスタジオ背景。装飾がなく最も軽量で、カードとテーブルが最も見やすい標準背景です。
- **おうち** : 木目の床・明るい壁・窓・棚・簡易的なソファのシルエットがある、暖かく落ち着いたリビング風の部屋です。
- **カジノ** : 深い赤のカーペット・暗めの壁・間接照明の光る帯・金色のトリム・奥に見える別テーブルのシルエットがある、架空のカードルームです（実在するカジノのロゴやブランドは使用していません）。
- **カードショップ** : 明るい床・白い壁・抽象的な色ブロックで表現したカード棚と商品・ポスター風パネル・複数の対戦テーブルがある、明るいカードショップの対戦スペースです（実在する商品画像やロゴは使用していません）。

いずれもThree.jsの基本形状（板・箱・円柱）と単色マテリアルのみで構成しており、外部のテクスチャ画像や3Dモデルファイルは使用していません。「無地」が最も軽量な構成で、低スペックなPCでもテーブル操作のフレームレートを優先できます。
...
│  │                       # 山札ビューア・盤面保存/読込・カメラ（斜め/真上切替）・テーマ/部屋背景選択・HandPanel（2D手札UI）・
│  │                       # OpponentHandRack（相手の手札枚数表示）・PlayerSwitchOverlay（プレイヤー交代確認画面）・ThreeContextBridge
│  │  └─ environments/     # 部屋背景コンポーネント（RoomShell共通土台 + Plain/Home/Casino/CardShop + Renderer）
│  └─ online/              # オンライン専用3Dテーブル・参加者一覧・招待URL・接続状態バナーなど
├─ routes/                 # MainMenuPage / BoxPage / DeckEditPage / PlayPage / PlaySetupPage（2人用テストプレイ設定） /
