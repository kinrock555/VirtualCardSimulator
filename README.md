...
このアプリはカードゲームの**ルールを一切処理しません**。ターン管理・フェーズ管理・コスト計算・ダメージ計算・勝敗判定などはすべてユーザー自身が行う前提です。アプリが管理するのは「カード画像」「デッキデータ」「3Dテーブル上のカードの状態（山札・手札・場・墓地・除外・束）」だけです。第4段階として、**2人用のミニマルなオンライン対戦モード**を追加しました（詳細は後述の「オンライン対戦モード」を参照）。第5段階では、実際にテストプレイする際の見やすさ・操作性を改善しました。手札は画面下部の専用2D UIへ変更し、3Dテーブルは厚み・フレーム・脚のある立体的なデザインへ、フィールドは明るく見やすく、テーブルの周囲には選択可能な部屋背景（無地・おうち・カジノ・カードショップ）を追加しています。1人用のオフライン機能・オンライン対戦モードの既存機能はすべてそのまま利用できます。
...
- 手札⇔フィールドの移動（手札カードをドラッグしてフィールドへ出す、フィールドカードを手札エリアへドラッグして戻す、右クリックメニューからも戻せる）
- テーブルリセット（確認ダイアログ付き、全カード回収＋シャッフル）
- 盤面（カードの配置・山札・手札・墓地・除外・束・テーマ・部屋背景・カメラ位置）の名前付き保存/読み込み/名前変更/削除
...
手札パネルのカードを左ドラッグすると、半透明のプレビューがマウスに追従します。3Dフィールド上に入ると、実際に置かれる位置に黄色い半透明のプレビューが表示されます。フィールド上でマウスを離すとそのカードが表向きでテーブルに置かれ、手札から取り除かれます。手札パネルの中で離した場合や、3Dフィールドの外で離した場合はカードが手札にとどまり、消えることはありません。
...
画面右上の「盤面を保存」ボタンを押すと、保存名を入力するダイアログが開きます。空の名前では保存できません。既に同じ名前の保存がある場合は上書き確認が表示されます。保存される内容は、使用デッキID・すべてのカードインスタンス・山札/墓地/除外/束の順序と位置・手札・選択中のテーブルテーマ・選択中の部屋背景・カメラ位置と注視点・作成/更新日時です。Three.jsのオブジェクト自体は保存されず、座標や角度、IDなどの純粋なデータのみが`localStorage`に保存されます。

## 保存した盤面の読み込み方法

「保存した盤面を開く」ボタンを押すと、保存済み盤面の一覧が表示されます。各項目に保存名・使用デッキ名（デッキが削除済みの場合はその旨を表示）・作成日時・更新日時・「読み込む」「名前変更」「削除」ボタンが並びます。「読み込む」を押すと確認ダイアログが表示され、承諾すると現在の盤面を破棄して保存データを復元します（カードの配置・山札/墓地/除外/束・手札・テーブルテーマ・部屋背景・カメラ位置も含めて復元されます）。保存データ内に存在しないカードIDや削除済みデッキが含まれていてもアプリは停止せず、不足分はスキップしたうえで通知が表示されます。**部屋背景を追加する前に保存された古いデータ**（`selectedRoomEnvironmentId`を含まないデータ）を読み込んだ場合はエラーにならず、自動的に「無地」背景として復元されます。
...
│  ├─ table/               # オフライン3Dテーブル(R3F): カード/山札/墓地/除外/束表示・ドラッグ・複数選択・各種右クリックメニュー・
│  │                       # 山札ビューア・盤面保存/読込・カメラ・テーマ/部屋背景選択・HandPanel（2D手札UI）・ThreeContextBridge
...
- `useTableStore` : オフラインのテストプレイ中のカードインスタンス（`Record<instanceId, CardInstance>`）、山札/墓地/除外/束（`CardStack[]`）、手札順（`string[]`）、選択（複数）/ドラッグ/各種右クリックメニュー/山札ビューアの状態、選択中テーブルテーマ・選択中の部屋背景・手札パネルの開閉状態（`localStorage`永続化、盤面自体は非永続）、手札→フィールドのドラッグ中プレビュー位置（非永続の一時状態）
