...
export function OnlineCardPreviewOverlay() {
  ...
  const instance = useOnlineStore((state) => (primaryInstanceId && state.table ? state.table.cardInstances[primaryInstanceId] : undefined));
  ...
  const isKnown = Boolean(instance?.cardId);
  const imageUrl = !instance ? undefined : instance.faceUp && isKnown ? (card?.imagePath ?? getCardBackUrl()) : getCardBackUrl();
  const label = !instance ? undefined : instance.faceUp && isKnown ? (card?.name ?? '不明なカード') : '裏向き / 非公開';
  ...
  return (
    <div className="card-preview-overlay">
      ...
      <div className="card-preview-overlay-name">{multiCount > 0 ? `${multiCount}枚選択中` : primaryInstanceId ? label : '未選択'}</div>
    </div>
  );
}
